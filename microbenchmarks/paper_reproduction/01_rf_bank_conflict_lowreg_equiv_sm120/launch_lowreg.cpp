...
int main(int argc, char **argv) {
    if (argc < 3) {
        printf("usage: %s kernel.cubin expected_sink\n", argv[0]);
        return 1;
    }

    const char *cubin_path = argv[1];
    float expected = std::atof(argv[2]);
    ...
    CUfunction func;
    CHECK_DRV(cuModuleGetFunction(&func, mod, "lowreg_kernel"));
    ...
    CHECK_DRV(cuMemAlloc(&d_guard, sizeof(float)));

    void *args[] = { &d_cycles, &d_sink, &d_guard };
    ...
    for (int i = 0; i < 50; i++) {
        ...
        CHECK_DRV(cuMemsetD32(d_guard, 0, 1));
        ...
        float diff = std::fabs(h_sink - expected);
        if (diff > 0.01f) ok = false;

        printf("run %02d: cycles = %llu, sink = %.6f, expected = %.6f, diff = %.6f\n",
               i, h_cycles, h_sink, expected, diff);
    }
