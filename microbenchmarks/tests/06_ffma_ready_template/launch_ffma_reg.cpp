...
int main(int argc, char **argv) {
    ...
    CUfunction func;
    CHECK_DRV(cuModuleGetFunction(&func, mod, "ffma_reg_kernel"));
    ...
    CHECK_DRV(cuMemAlloc(&d_in, 3 * sizeof(float)));

    float h_in[3] = {2.0f, 3.0f, 5.0f};
    CHECK_DRV(cuMemcpyHtoD(d_in, h_in, 3 * sizeof(float)));

    void *args[] = { &d_cycles, &d_sink, &d_in };
